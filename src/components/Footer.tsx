...
const Footer = () => {
  ...
  const contactInfo = [
    { 
      ...
      text: "123 Education Road, Academic City, Nigeria"
    },
    { 
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="container mx-auto px-4 py-12">
            ...
            <ul className="space-y-2">
              {quickLinks.map((link, index) => (
                <li key={index}>
                  <Link 
                    to={link.path}
                    className="hover:text-chenin transition flex items-center"
                  >
            ...
            <ul className="space-y-3">
              {contactInfo.map((info, index) => (
                <li key={index} className="flex items-start">
                  <span className="mr-3 mt-0.5 text-chenin">
                    {info.icon}
                  </span>
                  <span>{info.text}</span>
                </li>
        ...
        <div className="border-t border-white/20 pt-8 mt-8 flex flex-col md:flex-row justify-between items-center">
          ...
          <div className="flex flex-col md:flex-row md:space-x-6 text-sm">
            ...
            <Link to="/terms" className="hover:text-chenin transition">
              Terms of Service
            </Link>
          </div>
        </div>
