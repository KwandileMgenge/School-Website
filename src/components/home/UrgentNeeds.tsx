...
const UrgentNeeds: React.FC<UrgentNeedsProps> = ({
  ...
  theme = 'yellow',
  showProgress = false,
  collectionName = 'urgentNeeds',
  limit = 5,
}) => {
  const [urgentItems, setUrgentItems] = useState<UrgentItem[]>(items);
  const [loading, setLoading] = useState<boolean>(!items.length);
  ...
  useEffect(() => {
    // If items are provided via props, use them and don't fetch from Firebase
    if (items.length > 0) {
    ...
    const fetchUrgentNeeds = async () => {
      try {
        ...
        const querySnapshot = await getDocs(urgentQuery);
        const urgentData = querySnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data()
        })) as UrgentItem[];

        const limitedData = urgentData.slice(0, limit).map(item => ({
          ...item,
          amount: formatAmount(item.amount)
        }));

        setUrgentItems(limitedData);
        setError(null);
      } catch (err) {
        console.error('Error fetching urgent needs:', err);
        setError('Failed to load urgent needs. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <section className={`p-6 rounded-lg border-l-4 ${themeClasses[theme]} shadow-sm`}>
        ...
        <div className="text-center py-4">Loading urgent needs...</div>
      </section>
    );
  }

  if (error && urgentItems.length === 0) {
    return (
      <section className={`p-6 rounded-lg border-l-4 ${themeClasses[theme]} shadow-sm`}>
              ...
              {showProgress && (
                <div className="mt-1 w-full bg-white rounded-full h-2">
                  <div 
                    ...
                    style={{ width: item.funded ? '100%' : '30%' }}
                  ></div>
                </div>
              )}
            </div>
